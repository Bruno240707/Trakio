...
import DoughnutChart from "../../Componentes/DoughnutChart";
...
const DashboardsInd = ({ empleados }) => {
  ...
  useEffect(() => {
    fetch(`/api/getWorkers?filtro=${filtro}&sucursal=${sucursalSeleccionada}`, { credentials: "include" })
      ...
      .catch((err) => console.error("Error al cargar la API de empleados:", err));
  ...
  useEffect(() => {
    const fetchSucursales = async () => {
      try {
        ...
        if (!res.ok) throw new Error("Error en la respuesta de la API de sucursales");
  ...
  // Buscar sucursal actual del empleado
  ...
  useEffect(() => {
    ...
    fetch(`/api/eventsEntradasSalidasByWorkerAndDate/${workerId}?year=${year}&month=${month}&day=${day}`, {
      credentials: "include",
    })
      .then((res) => res.json())
      .then((data) => setBarData(data))
      .catch((err) => console.error("Error al cargar barData:", err));
  ...
  useEffect(() => {
    ...
    fetch(`/api/attendanceDoughnut/${workerId}?year=${year}&month=${month}`, { credentials: "include" })
      ...
      .catch((err) => console.error("Error al cargar doughnutData:", err));
  }, [workerId, year, month]);
      ...
      {workerId ? (
            ...
            <div className="employee-header">
              {workerActual && sucursalActual ? (
                <>
                  ...
                  <p className="employee-sucursal">{sucursalActual.nombre}</p>
                </>
              ) : (
                <p>Cargando...</p>
              )}
        ...
        <main className="main-content empty-dashboard">
          <div className="empty-container">
            ...
            <p>
              Para ver su información y gráficos, haga clic en un empleado de la lista
              a la izquierda.
            </p>
